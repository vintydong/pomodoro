...
function AppContent() {
  const { mode, timeLeft, isActive, toggleTimer, resetTimer, skipTimer, history, settings } = usePomodoro();
    ...
    <div className="min-h-screen bg-background text-foreground transition-colors duration-300">
      ...
      <main className="container mx-auto px-4 pt-24 pb-12 max-w-4xl space-y-12">
        ...
        <section className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <h2 className="text-2xl font-bold tracking-tight">Productivity Stats</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="p-6 rounded-xl border bg-card text-card-foreground shadow-sm flex flex-col justify-center">
              <div className="text-center space-y-2">
                <div className="text-4xl font-bold">{history.filter(s => s.completed && s.mode === 'focus').length}</div>
